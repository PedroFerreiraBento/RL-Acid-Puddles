...
class GridworldV1Window(arcade.Window):
    ...
    def __init__(
        ...
        """
        # Compute an initial fitted cell size so the board fits on the screen
        init_cs = self._compute_fitted_cell_size(env.cols, env.rows, viz.cell_size)
        viz.cell_size = init_cs
        width = env.cols * viz.cell_size
        height = env.rows * viz.cell_size
        super().__init__(width=width, height=height, title="Gridworld V1 (Planning)", resizable=True)
        ...
        # Optional obstacles (acid puddles)
        self.tex_obstacle = arcade.load_texture(str(ASSETS / "obstacles.png"))
        ...
        # Keyboard: handle window-specific keys (F11) then delegate to env
        def _on_key(symbol: int, modifiers: int) -> None:
            if symbol == pygkey.F11:
                self.toggle_fullscreen_fit()
                return
            # Delegate everything else to the environment if it exposes handler
            try:
                self.env.on_key_press(symbol)  # type: ignore[attr-defined]
            except Exception:
                pass
        self.on_key_press = _on_key  # type: ignore[assignment]

    # --- Display helpers ---
    def _compute_fitted_cell_size(self, cols: int, rows: int, desired: int) -> int:
        """Return a cell size that fits the board in the current display.

        Keeps cells square (no distortion). Uses up to ~95% of available size.
        """
        try:
            disp_w, disp_h = arcade.get_display_size()
        except Exception:
            # Fallback if display size not available
            disp_w, disp_h = 1280, 720
        margin = 0.95
        max_w = int(disp_w * margin)
        max_h = int(disp_h * margin)
        max_cs_w = max(1, max_w // max(1, cols))
        max_cs_h = max(1, max_h // max(1, rows))
        fit_cs = max(1, min(desired, max_cs_w, max_cs_h))
        return fit_cs

    def toggle_fullscreen_fit(self) -> None:
        """Toggle fullscreen and refit the board without distortion."""
        try:
            self.set_fullscreen(not self.fullscreen)
        except Exception:
            return
        # Recompute fitted cell size for current display mode
        new_cs = self._compute_fitted_cell_size(self.env.cols, self.env.rows, self.viz.cell_size)
        if new_cs != self.viz.cell_size:
            self.viz.cell_size = new_cs
            self._rebuild_tiles()
            self._move_sprite_to_state()
        # Ensure window size matches grid size under new cell size
        width = self.env.cols * self.viz.cell_size
        height = self.env.rows * self.viz.cell_size
        try:
    ...
    def on_resize(self, width: int, height: int):
        """Auto-fit the board when the window is resized, preserving aspect.

        We recompute a cell size that fits the current display (not stretching
        cells), then snap the window to exactly grid_size * cell_size to avoid
        distortion of sprites and textures.
        """
        try:
        ...
        if new_cs != self.viz.cell_size:
            ...
            self._move_sprite_to_state()
        try:
            self.set_size(self.env.cols * self.viz.cell_size, self.env.rows * self.viz.cell_size)
        except Exception:
            pass
    ...
    def on_draw(self):
        ...
        # Draw obstacles over floor before walls/goal/heatmap
    ...
    def on_update(self, dt: float):
        ...
        if self._accum >= self.viz.step_interval:
            ...
            if self.done and self.env.state() == self.env.cfg.goal:
                ...
                self._rebuild_tiles()
                self._clear_heatmap()
                # Clear per-state action debug cache to avoid unbounded growth across episodes
                try:
                    self.sa_debug.clear()
                except Exception:
                    pass
    ...
    def _draw_heatmap(self):
        """Draw a translucent yellow heatmap per-visit overlay over traversable cells."""
        cs = self.viz.cell_size
        walls = getattr(self.env, "walls", set())
        # Draw translucent yellow squares with alpha proportional to visit frequency
        for y in range(1, self.env.rows - 1):
            for x in range(1, self.env.cols - 1):
                ...
                top = bottom + cs
                # Yellow tone (gold)
                arcade.draw_lrbt_rectangle_filled(left, right, bottom, top, (255, 215, 0, alpha))
    ...
    def _rebuild_tiles(self):
        ...
        s_obst = cs / max(1, self.tex_obstacle.width)

        # Explicitly clear previous lists to free GPU buffers before recreating
        if hasattr(self, "floor_sprites") and isinstance(self.floor_sprites, arcade.SpriteList):
            try:
                self.floor_sprites.clear()
            except Exception:
                pass
        if hasattr(self, "wall_sprites") and isinstance(self.wall_sprites, arcade.SpriteList):
            try:
                self.wall_sprites.clear()
            except Exception:
                pass
        if hasattr(self, "goal_sprites") and isinstance(self.goal_sprites, arcade.SpriteList):
            try:
                self.goal_sprites.clear()
            except Exception:
                pass
        if hasattr(self, "obstacle_sprites") and isinstance(self.obstacle_sprites, arcade.SpriteList):
            try:
        ...
        # Obstacles (acid puddles) placed on top of floor tiles, not walls
        obstacles = getattr(self.env, "obstacles", set())
        for (ox, oy) in obstacles:
            cx = ox * cs + cs / 2
            cy = oy * cs + cs / 2
            spr = arcade.Sprite()
            spr.texture = self.tex_obstacle
            spr.center_x = cx
            spr.center_y = cy
            spr.scale = s_obst
            self.obstacle_sprites.append(spr)
