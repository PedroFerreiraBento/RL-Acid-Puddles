...
def sarsa_train(
    ...
    def _greedy_tiebreak(s: Tuple[int, int], prev_a: int | None = None) -> int:
        ...
        # Add goal proximity and puddle-avoidance biases to reduce rotational pong
        goal = env.cfg.goal
        hazard = float(getattr(env.rew, 'hazard', 0.0))
        puddle_bias_mag = 0.1 * abs(hazard)  # e.g., -0.4 when hazard=-4
        for i in valids:
            s2 = _next_from(s, i)
            # Prefer moves that reduce Manhattan distance to goal
            d1 = abs(s[0] - goal[0]) + abs(s[1] - goal[1])
            d2 = abs(s2[0] - goal[0]) + abs(s2[1] - goal[1])
            if d2 < d1:
                q_hat[i] += 0.05
            elif d2 > d1:
                q_hat[i] -= 0.05
            # Avoid stepping on puddles
            if s2 in getattr(env, 'obstacles', set()):
                q_hat[i] -= puddle_bias_mag
        max_q = np.max(q_hat)
    ...
    def eps_greedy(s: Tuple[int, int], eps: float, prev_a: int | None = None) -> int:
        if rng.random() < eps:
            ...
            # Add goal proximity and puddle-avoidance biases inside the exploration pool
            goal = env.cfg.goal
            hazard = float(getattr(env.rew, 'hazard', 0.0))
            puddle_bias_mag = 0.1 * abs(hazard)
            # Prefer actions that reduce distance to the goal; avoid puddles
            scored_pool = []
            for a_opt in pool:
                s2_opt = _next_from(s, a_opt)
                d1 = abs(s[0] - goal[0]) + abs(s[1] - goal[1])
                d2 = abs(s2_opt[0] - goal[0]) + abs(s2_opt[1] - goal[1])
                score = 0.0
                if d2 < d1:
                    score += 0.05
                elif d2 > d1:
                    score -= 0.05
                if s2_opt in getattr(env, 'obstacles', set()):
                    score -= puddle_bias_mag
                scored_pool.append((a_opt, score))
            if scored_pool:
                max_s = max(sc for _, sc in scored_pool)
                pool = [a_opt for a_opt, sc in scored_pool if sc == max_s]
