...
class GWConfig:
    ...
    # Optional: non-blocking obstacles (e.g., acid puddles). The movement is
    # allowed onto these cells, but a hazard penalty is applied.
    obstacles: Set[Tuple[int, int]] = field(default_factory=set)
    hazard_reward: float = 0.0
...
def step_model(cfg: GWConfig, s: Tuple[int, int], a: int) -> Tuple[Tuple[int, int], float, bool]:
    ...
    if done:
        ...
        # Apply hazard penalty if stepping onto a non-blocking obstacle
        if not bumped and s2 in getattr(cfg, "obstacles", set()):
            r += float(getattr(cfg, "hazard_reward", 0.0))
