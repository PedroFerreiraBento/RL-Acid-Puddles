...
class Rewards:
    ...
    # Extra penalty when stepping on a non-blocking obstacle (acid puddle)
    hazard: float = -4.0
...
class GridCfg:
    ...
    # Number of non-blocking obstacles (acid puddles) to place in the interior
    obstacles_count: int = 8
    # Fraction of obstacles to be placed along the current shortest path
    # (0.0 -> all off-path, 1.0 -> all on-path). Keeps start/goal clear.
    obstacles_on_path_ratio: float = 0.3
    # For maze layout, carve side-lane detours around this fraction of
    # on-path obstacles so the agent can go around them.
    detour_ratio: float = 0.5
...
class GridworldV1:
    ...
    def __init__(self, cfg: GridCfg):
        ...
        # Non-blocking obstacles (acid puddles)
    ...
    def from_json(path: Path) -> "GridworldV1":
        ...
        gc = GridCfg(
            ...
            obstacles_count=int(data.get("obstacles_count", 8)),
            obstacles_on_path_ratio=float(data.get("obstacles_on_path_ratio", 0.3)),
            detour_ratio=float(data.get("detour_ratio", 0.5)),
        )
    ...
    def _build_walls(self) -> None:
        ...
        if self.cfg.layout == "corridor":
            ...
            self._pos = self.cfg.start
        elif self.cfg.layout == "open":
            # No interior walls; keep only borders
            pass
        elif self.cfg.walls:
            for x, y in self.cfg.walls:
                if 1 <= x < self.cols - 1 and 1 <= y < self.rows - 1:
                    self.walls.add((x, y))
        # After walls are known, generate obstacles away from start/goal and walls
        self._build_obstacles()
        # In maze layout, carve optional detours around some on-path obstacles
    ...
    def _build_obstacles(self) -> None:
        """Place non-blocking obstacles (acid puddles) on free interior cells.

        Obstacles do not block movement but will impose an additional hazard
        penalty when stepped on. They are kept clear of start/goal and walls.
        """
        self.obstacles.clear()
        # Free interior cells (not walls/border/start/goal)
        free_cells: List[Tuple[int, int]] = [
            (x, y)
            for y in range(1, self.rows - 1)
            for x in range(1, self.cols - 1)
            if (x, y) not in self.walls and (x, y) not in {self.cfg.start, self.cfg.goal}
        ]
        if not free_cells:
            return
        count = max(0, int(self.cfg.obstacles_count))
        count = min(count, len(free_cells))

        # Compute a shortest path from start to goal through free cells (BFS)
        def bfs_path(start: Tuple[int, int], goal: Tuple[int, int]) -> List[Tuple[int, int]]:
            if start == goal:
                return [start]
            q = deque([start])
            prev: Dict[Tuple[int, int], Optional[Tuple[int, int]]] = {start: None}
            free = set(free_cells) | {start, goal}
            ...
            while q:
                ...
                for dx, dy in dirs:
                    nx, ny = cx + dx, cy + dy
                    if 1 <= nx < self.cols - 1 and 1 <= ny < self.rows - 1 and (nx, ny) in free and (nx, ny) not in prev:
                        prev[(nx, ny)] = (cx, cy)
                        q.append((nx, ny))
            if goal not in prev:
                return []
        ...
        path_cells = set(bfs_path(self.cfg.start, self.cfg.goal))

        # Partition free cells into on-path vs off-path
        on_path = [c for c in free_cells if c in path_cells and c not in {self.cfg.start, self.cfg.goal}]
        off_path = [c for c in free_cells if c not in path_cells]

        # Decide how many obstacles go on the shortest path
        ratio = max(0.0, min(1.0, float(self.cfg.obstacles_on_path_ratio)))
        if count <= 0:
            k_on = 0
            k_off = 0
        else:
            k_on = int(round(count * ratio))
            # Guarantee at least 1 on-path obstacle if possible
            if len(on_path) > 0:
                k_on = max(1, k_on)
                k_on = min(k_on, len(on_path), count)
            else:
                k_on = 0
            k_off = max(0, min(count - k_on, len(off_path)))

        # Sample without replacement
        chosen: List[Tuple[int, int]] = []
        if k_on > 0 and on_path:
            idxs_on = self.rng.choice(len(on_path), size=k_on, replace=False)
            for i in (idxs_on if isinstance(idxs_on, np.ndarray) else [idxs_on]):
                chosen.append(on_path[int(i)])
        if k_off > 0 and off_path:
            idxs_off = self.rng.choice(len(off_path), size=k_off, replace=False)
            for i in (idxs_off if isinstance(idxs_off, np.ndarray) else [idxs_off]):
                chosen.append(off_path[int(i)])

        # Ensure at least one obstacle sits on the path (if possible)
        forced_on: Optional[Tuple[int, int]] = None
        if not any(c in on_path for c in chosen) and on_path:
            # Force one on-path obstacle
            idx_force = self.rng.integers(0, len(on_path))
            forced_on = on_path[int(idx_force)]
            chosen.append(forced_on)
            # If we exceeded the desired count, drop a random off-path selection
            if len(chosen) > count:
                # Remove one OFF-PATH obstacle (never remove the forced one)
                off_only = [c for c in chosen if (c not in on_path) and (c != forced_on)]
                if off_only:
                    drop_idx = int(self.rng.integers(0, len(off_only)))
                    chosen.remove(off_only[drop_idx])

        self.obstacles.update(chosen)

        # Cache path cells for detour step
        self._last_shortest_path = path_cells
        self._forced_on_path_obstacle = forced_on

    def _carve_detours(self) -> None:
        """Widen the maze locally around some on-path obstacles to allow bypass.

        Strategy: For obstacles that lie in a 1-wide corridor, carve a
        1-cell-wide side lane that connects the two corridor halves, creating a
        way around the obstacle without removing the obstacle itself.
        """
        if not hasattr(self, "_last_shortest_path"):
            return
        path_cells: Set[Tuple[int, int]] = set(getattr(self, "_last_shortest_path", set()))
        on_path_obstacles = [o for o in self.obstacles if o in path_cells]
        if not on_path_obstacles:
            return
        # Decide how many to detour
        ratio = max(0.0, min(1.0, float(self.cfg.detour_ratio)))
        k = int(round(len(on_path_obstacles) * ratio))
        # Guarantee at least one detour
        if k <= 0:
            k = 1
        idxs = self.rng.choice(len(on_path_obstacles), size=k, replace=False)
        targets = [on_path_obstacles[int(i)] for i in (idxs if isinstance(idxs, np.ndarray) else [idxs])]
        # Ensure we prioritize the forced on-path obstacle if present
        forced = getattr(self, "_forced_on_path_obstacle", None)
        if forced and forced in on_path_obstacles and forced not in targets:
            targets[0] = forced

        def is_free(c: Tuple[int,int]) -> bool:
            x,y=c
            return 1 <= x < self.cols-1 and 1 <= y < self.rows-1 and (x,y) not in self.walls

        carved_any = False
        for ox, oy in targets:
            # Carve a compact 3x3 ring around the obstacle to guarantee a bypass
            ...
            for c in ring:
                if c in self.walls:
                    ...
                    any_local = True
            carved_any = carved_any or any_local

        # Final guarantee: if nothing got carved (very unlikely), open one neighbor of the forced obstacle
        if not carved_any:
            fo = getattr(self, "_forced_on_path_obstacle", None)
            if fo:
                ox, oy = fo
                for dx, dy in ((1,0),(-1,0),(0,1),(0,-1)):
                    cx, cy = ox + dx, oy + dy
                    if 1 <= cx < self.cols-1 and 1 <= cy < self.rows-1 and (cx, cy) in self.walls:
    ...
    def compute_policies(self) -> None:
        ...
        gwcfg = GWConfig(
            ...
            hazard_reward=float(self.rew.hazard),
        )
    ...
    def step(self, action: Action) -> Tuple[Tuple[int, int], float, bool, Dict[str, Any]]:
        ...
        # Hazard penalty for stepping on a non-blocking obstacle
        if self._pos in getattr(self, "obstacles", set()) and self._pos != self.cfg.goal:
            reward += self.rew.hazard
    ...
    def _ui_reset_episode(self) -> None:
        ...
        try:
            ...
            # Clear per-state action debug cache to prevent growth across resets
            try:
                w.sa_debug.clear()
            except Exception:
                pass
